...
def readfile(_endpoint, filepath, _ruid, _rgid):
    ...
    try:
        ...
        initiatefiledownloadres = credentials['cs3stub'].InitiateFileDownload(request = req, metadata = [('x-access-token', credentials['token'])])
...
def writefile(_endpoint, filepath, content, noversion=0):
   ...
   # size = len(content)
    ...
    try:
        ...
        headers = {'X-Reva-Transfer': res1.token, 'Authorization': 'Basic ZWluc3RlaW46cmVsYXRpdml0eQ=='}
        ...
        uploader = my_client.uploader(file_path='src/example.txt', chunk_size=200, metadata = metadata)
