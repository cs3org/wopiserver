...
class TestStorage(unittest.TestCase):
    ...
    def test_lock(self):
        ...
        localVariable = self.storage.getlock(self.endpoint, self.homepath + '/testlock', self.userid)
        self.assertIsInstance(localVariable, dict)
        self.assertEqual(localVariable['lock_id'], 'testlock')
        self.assertEqual(localVariable['app_name'], 'myapp')
        self.assertIsInstance(localVariable['expiration'], dict)
        self.assertIsInstance(localVariable['expiration']['seconds'], int)
        with self.assertRaises(IOError) as context:
    ...
    def test_refresh_lock(self):
        ...
        localVariable = self.storage.getlock(self.endpoint, self.homepath + '/testrlock', self.userid)
        self.assertIsInstance(localVariable, dict)
        self.assertEqual(localVariable['lock_id'], 'testlock2')
        self.assertEqual(localVariable['app_name'], 'myapp')
        self.assertIsInstance(localVariable['expiration'], dict)
        self.assertIsInstance(localVariable['expiration']['seconds'], int)
        with self.assertRaises(IOError) as context:
    ...
    def test_expired_locks(self):
        ...
        time.sleep(3)
        localVariable = self.storage.getlock(self.endpoint, self.homepath + '/testelock', self.userid)
        self.assertEqual(localVariable, None)
        ...
        localVariable = self.storage.getlock(
            self.endpoint, self.homepath + '/testelock', self.userid
        )
        self.assertIsInstance(localVariable, dict)
        self.assertEqual(localVariable['lock_id'], 'testlock3')
        time.sleep(3)
        with self.assertRaises(IOError) as context:
